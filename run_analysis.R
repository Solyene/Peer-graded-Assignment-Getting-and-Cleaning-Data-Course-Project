...
a <- fread('./data/UCI HAR Dataset/train/X_train.txt')

c <- fread('./data/UCI HAR Dataset/train/y_train.txt')

d <- cbind(a, c)

e <- fread('./data/UCI HAR Dataset/train/subject_train.txt')

f <- cbind(e, d)

f <- cbind(e, d)

x <- read.table('./data/UCI HAR Dataset/features.txt', col.names = c('MeasureID', 'MeasureName'))
...
#create test dataset
b <- fread('./data/UCI HAR Dataset/test/X_test.txt')

k <- fread('./data/UCI HAR Dataset/test/y_test.txt')

l <- cbind(k, b)

m <- fread('./data/UCI HAR Dataset/test/subject_test.txt')
...
lab <- read.table("./data/UCI HAR Dataset/activity_labels.txt", col.names = c('activityID', 'activityName'))
